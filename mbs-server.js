...
function loadDeviceConfiguration() {
  ...
  Object.keys(output).forEach(function (device) {
    ...
    Object.keys(output[device]["subscribe"]).forEach(function (attribute) {
      Object.keys(output[device]["subscribe"][attribute]).forEach(function (
        sub
      ) {
        let data = {};
        data["device"] = device;
        ...
        if (
          !!output[device]["subscribe"][attribute][sub] &&
          !!output[device]["subscribe"][attribute][sub]["command"]
        )
          data["command"] =
            output[device]["subscribe"][attribute][sub]["command"];
        ...
        winston.debug(
          ...
          sub,
          subscribe[sub][device][attribute]["device"],
          subscribe[sub][device][attribute]["attribute"],
          subscribe[sub][device][attribute]["command"]
        );
  ...
  winston.info(
    "============================ALL POSSIBLE SUBSCRIPTIONS FROM ALL EXTERNAL DEVICES ==========================================="
  );
    ...
    Object.keys(subscribe[subs]).forEach(function (dev) {
      Object.keys(subscribe[subs][dev]).forEach(function (attribute) {
        winston.info(
          "Subscription %s\t[%s],[%s],[%s]",
          subs,
          subscribe[subs][dev][attribute]["device"],
          subscribe[subs][dev][attribute]["attribute"],
          JSON.stringify(
            !!subscribe[subs][dev][attribute]["command"]
              ? subscribe[subs][dev][attribute]["command"]
              : ""
          )
        );
...
function handlePushEvent(req, res) {
  ...
  if (
    !!devices[device] &&
    !!devices[device]["publish"] &&
    !!devices[device]["publish"][attribute]
  ) {
    retain = !!devices[device].retain ? devices[device].retain : retain;
    winston.debug(
      "ST** --> MQTT: [%s][%s][%s]\t[%s][%s][Retain:%s]",
      req.body.name,
      req.body.type,
      req.body.value,
      pub,
    ...
    Object.keys(devices[device]["publish"][attribute]).forEach(function (pub) {
      value =
        ...
        !!devices[device]["publish"][attribute][pub].command[value]
          ? devices[device]["publish"][attribute][pub].command[value]
          : value;
      topic = pub;
      winston.info(
...
function mqttPublish(device, attribute, topic, value, retainflag, res) {
  ...
  } else if (!!publications[topic][device])
    publications[topic][device]["command"] = value;
  var sub = isSubscribed(topic);
  if (
    !!subscribe &&
    !!subscribe[sub] &&
    !!subscribe[sub][device] &&
    !!subscribe[sub][device][attribute]
  ) {
    winston.warn(
      "POSSIBLE LOOP. Device[Attribute] %s[%s] is publishing to Topic %s while subscribed to Topic %s",
      device,
      attribute,
      topic,
      sub
    ...
    winston.warn(
      "POSSIBLE LOOP. Device %s is publishing to Topic %s while subscribed to Topic %s",
      device,
      topic,
      sub
    );
...
function processSubscriptions(req) {
    ...
    req[property].forEach(function (device) {
      ...
      if (!!devices && !!devices[device]) {
        if (
          ...
          !!devices[device]["subscribe"][property]
        ) {
          Object.keys(devices[device]["subscribe"][property]).forEach(function (
            sub
          ) {
  ...
  // Subscribe to events
  winston.info(
    "===================================ACTUAL SUBSCRIPTIONS REQUESTED FROM SMARTAPP ============================================"
  );
...
function isSubscribed(topic) {
  ...
  for (let i = 0; i < subscriptions.length; i++) {
    ...
    } else if (mqttWildcard(topic, subscriptions[i]) != null)
      topics.push(subscriptions[i]);
  }
  return topics;
}
...
function parseMQTTMessage(incoming, message) {
  ...
  if (!topics) {
    winston.warn(
      "%s-%s not subscribed. State error. Ignoring. ",
      incoming,
      contents
    );
    return;
  ...
  if (topics.length > 1)
    winston.info(
      "Incoming topic maps to multiple subscriptions: %s = %s",
      incoming,
      topics
    );
      ...
      Object.keys(subscribe[topic]).forEach(function (name) {
        ...
        if (!!devices[name]) {
          Object.keys(subscribe[topic][name]).forEach(function (attribute) {
            ...
            value = contents;
            if (
              !!subscribe[topic][name][attribute]["command"] &&
              !!subscribe[topic][name][attribute]["command"][contents]
            )
          ...
          var topicReadState = getTopicFor(device, property, TOPIC_READ_STATE),
            ...
            cmd =
              !pieces[2] ||
              (pieces[2] && pieces[2] === config.mqtt[SUFFIX_COMMAND]);
          // Commented out because we want none of this logic.
...
function postRequest(topic, contents, device, property, value, cmd, incoming) {
  ...
  if (
    ...
    !!publications[incoming][device] &&
    !!publications[incoming][device][property]
  ) {
    winston.error(
      "Incoming %s for attribute %s for device %s is also being published to: [%s][%s][%s].\nIgnoring: %s = %s",
      incoming,
      property,
      device,
      device,
  ...
  if (!topic.match(/[*+]/)) {
    ...
    winston.info(
      "MQTT  --> MQTT[WildCard] - Topic: [%s][%s]\t[%s][%s]",
      incoming,
      msg,
      topic,
      msg
    ...
    winston.info(
      "MQTT[WildCard] --> ST - Topic: [%s][%s]\t[%s][%s][%s]",
      topic,
      msg,
      device,
      property,
    ...
    function loadFromDisk(next) {
      ...
      if (!!st_request) {
        try {
          ...
          winston.error(ex);
          winston.info(
            "Could not restore subscriptions. Please rebuscribe in IDE, continuing"
          );
          return;
        }
      }
    ...
    function setupApp(next) {
      ...
      app.use(
        expressWinston.logger({
          format: logFormat,
          msg:
            "HTTP /{{req.method}} {{req.url}} host={{req.hostname}} --> Status Code={{res.statusCode}} ResponseTime={{res.responseTime}}ms",
      ...
      app.use(
        expressWinston.errorLogger({
          format: logFormat,
          msg:
            " [{{res.statusCode}} {{req.method}}] {{err.message}}\n{{err.stack}}",
